...
describe('AITU_SE2315_Arlen_Modified', function () {
	...
	beforeEach(async function () {
		...
		Token = await ethers.getContractFactory('AITU_SE2315_Token')
	...
	it('Should emit TransactionDetails event on transfer', async function () {
		...
		const tx = await token
			.connect(addr1)
			.transferWithDetails(addr2.address, transferAmount)
		const receipt = await tx.wait()

		// Extract event logs
		const event = receipt.logs.find(
			log => log.eventName === 'TransactionDetails'
		)
	...
	it('Should fail to transfer tokens if balance is insufficient', async function () {
		...
		await expect(
			token.connect(addr1).transferWithDetails(addr2.address, largeAmount)
